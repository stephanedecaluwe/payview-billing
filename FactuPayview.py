...
config_path = r'C:/Users/sdecaluwe/Desktop/factupayview/code/Inputs/'

if str(os.listdir(config_path)).find("parametres.ini") < 0:
    print("pas de fichier de parametres.ini")
    exit(0)
    
# FICHIER_CORRESPONDANCE_NOMS_CLIENTS = r"//Frprfil/data/FranceTeam/Marketing&Communication/14. Projets/Passeport/Billing/01.Inputs/correspPayViewPassPort.xlsx"    
for line in csv.reader(open(config_path+"parametres.ini",encoding='latin-1'),delimiter="="):
    if len(line)>1:
        print(line[:line[0].find("=")])
        if line[0].find("PAYVIEW_ADMIN_EMAIL") == 0:
            PAYVIEW_ADMIN_EMAIL=line[1][1:-1]
        if line[0].find("PAYVIEW_ADMIN_MDP") == 0:
            PAYVIEW_ADMIN_MDP=line[1][1:-1]
        if line[0].find("CLIENTS_IGNORES") == 0:
            CLIENTS_IGNORES=line[1][3:-1]
        if line[0].find("FICHIER_SIM_PRET") == 0:
            FICHIER_SIM_PRET=line[1][3:-1]
        if line[0].find("FICHIER_CORRESPONDANCE_NOMS_CLIENTS") == 0:
	        FICHIER_CORRESPONDANCE_NOMS_CLIENTS=line[1][3:-1]
        if line[0].find("DOSSIER_GENERATION_RESULTATS") == 0:
            DOSSIER_GENERATION_RESULTATS=line[1][3:-1]
...
def lectureDossier(isPayView:bool, directory:str):
    ...
    def _lectureFichierSim(isPayView:bool, filePath:str,fileName:str):
        ...
        for l in readCsvOrExcel(filePath,forceEncoding='utf_16' if isPayView else None):
            ...
            if not resF:
                tailleForfaitSIM_MB = {'1024':1,'2048':2,'5120':5,'10240':10,'51200':5,'102400':100}.get(nomForfait,None)
...
def génèreFactu(année:int,mois:int):
    ...
    #PASSPORT_ZIP_DIRECTORY      =   r"C:\Users\sdecaluwe\Desktop\factupayview\code\Generated\20210701_11h13_facturation_juin_2021\DataPassPort"
    #PAYVIEW_DATA_DIRECTORY      =    r"C:\Users\sdecaluwe\Desktop\factupayview\code\Generated\20210701_11h13_facturation_juin_2021\DataPayView"
    PASSPORT_ZIP_DIRECTORY      = None
    PAYVIEW_DATA_DIRECTORY      = None

    #Récupération des données de PassPort
    if not PASSPORT_ZIP_DIRECTORY:
        PASSPORT_ZIP_DIRECTORY = os.path.join(dossierGénéCetteFactu,"DataPassPort")
        os.makedirs(PASSPORT_ZIP_DIRECTORY, mode=0o777, exist_ok=False)
        ap = DownloadBillingAPI(PASSPORT_ZIP_DIRECTORY, mois,année)
        ap.RécupèreFichiersFactu(ignoredListMinuscule=NomClientsIgnorésMinuscules)
    ...
    print("globalXlFile")
    print(globalXlFile)
    #mois = int(datetime.now().month)
    #année = int(datetime.now().year)
    ...
    #else:
    #   mois=mois-1
    #os.system('python commandeSAP.py '+globalXlFile+' '+str(mois)+' '+str(année))
